...
export default function Card2({ array, index, handle }) {
  const { title, excerpt, date, category, image, readTime, slug, id, author } = array || {};
  const { name, avatar, role } = array.author || {};
  console.log(slug);
  

  return (
    <article onClick={()=>{return <Avatars/>}} className="group card overflow-hidden">
